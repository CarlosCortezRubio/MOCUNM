    ...
    switch (tipo) {
        ...
        case "6":
            cuerpo = "<div class='container'>" +
                ...
                "<button class='btn btn-success' onclick='AgregarRespuesta(" + tipo + "," + '"#p' + preg + '"' + "," + '"#Resp' + preg + '"' + ");' type='button'>+</button>" +
                "</div>" +
                "<input type='file' id='Resp" + preg + "' class='form-control' placeholder='Ingrese una alternativa' aria-label='' aria-describedby='basic-addon1'>" +
                "</div>" +
                "<div class='col-2' ></div>" +
                "</div>" +
                "</div>" +
...
function AgregarRespuesta(tipo, idpreg, input) {
    ...
    switch (tipo) {
        ...
        case 6:
            ...
            if ($(input).get().files && $(input).get().files[0]) {
                var reader = new FileReader();
                reader.onload = function(e) {
                    cuerpo = "<input type='radio' name='resp" + idpreg + "' class='respuesta1 form-check-input'>" +
                        "<input type='file' id='inp" + cont + "' class='respuesta1 form-check-label' disabled  value='" + e.target.result + "'>" +
                        "<img src='" + e.target.result + "' alt='Pregunta sin imagen'>";


                }
                reader.readAsDataURL($(input).get(0).target.files[0]);
            } else {
                alert($(input).get(0).files[0]);
            }
            // cuerpo = "<input type='radio' name='resp" + idpreg + "'class='respuesta1 form-check-input'>" +
            //  "<input type='file' id='inp" + cont + "' class='respuesta1 form-check-label' disabled  value='" + $(input).val() + "'>" +
            //  "<img src='" + $(input).val() + "' alt='Pregunta sin imagen'>";
            break;
    }
    var pie = "</div>" +
